...
namespace SettingsModelUnitTests
{
    class KeyBindingsTests : public JsonTestClass
    {
        ...
        TEST_METHOD(SendInputImmediateEnterStaysDistinct);
    ...
    void KeyBindingsTests::SendInputImmediateEnterStaysDistinct()
    {
        const auto actionMap = winrt::make_self<implementation::ActionMap>();
        actionMap->LayerJson(VerifyParseSucceeded(R"([ { "command": { "action": "sendInput", "input": "just some input" }, "keys": ["ctrl+c"] } ])"), OriginTag::User);
        actionMap->LayerJson(VerifyParseSucceeded(R"([ { "command": { "action": "sendInput", "input": "just some input", "submitEnter": true }, "keys": ["ctrl+shift+c"] } ])"), OriginTag::User);
